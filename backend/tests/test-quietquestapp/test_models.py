...
# We'll use classes because they allow us to neatly group tests together(Grouping here the different tests for the file 'models.py').
...
class Test_LocationsModel:
    ...
    # we can then access the LocationsFactory in lower case and utilizing the underscore between locations and factory. That's the kind of format that's going to happen if we would call another registered factory
    def test_LocationsModel_str_return(self, locations_factory):
        # Get the expected coordinates string
        coordinates = locations_factory()
        assert coordinates.__str__() == "40.86333333,-73.92777777"
