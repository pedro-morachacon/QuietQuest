...
def run():
    ...
    for df_chunk in pd.read_csv('quietquestapp/Noise_Lat_Long.csv', chunksize=chunksize):
        ...
        with open("quietquestapp/noise_model.pkl", "rb") as pickle_file:
            ...
            values = [
                NoiseLocations(long=x_var[0], lat=x_var[1], hour=x_var[2], weekday=x_var[3],
    ...
    print(len(NoiseLocations.objects.all()))
    end = time.time()
