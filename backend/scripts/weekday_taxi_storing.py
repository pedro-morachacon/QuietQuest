...
def run():
    ...
    chunksize = 30000
    for df_chunk in pd.read_csv('quietquestapp/Taxi_Weekday_Lat_Long.csv', chunksize=chunksize):
        start2 = time.time()
        ...
        with open("quietquestapp/weekday_test_model.pkl", "rb") as pickle_file:
            ...
            values = [
                TaxiWeekdayLocations(long=x_var[0], lat=x_var[1], hour=x_var[2], count=prediction.item())
    ...
    print(len(TaxiWeekdayLocations.objects.all()))
    end = time.time()
